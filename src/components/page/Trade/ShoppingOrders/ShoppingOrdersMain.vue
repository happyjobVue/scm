      ...
      <tbody>
        <template v-if="shoppingOrdersList">
          <template v-if="shoppingOrdersList.shoppingCnt > 0">
            <tr v-for="shoppingOrders in shoppingOrdersList.shoppingList"
            ...
            >
              ...
              <td>{{ shoppingOrders.requestsReturnDate ? shoppingOrders.requestsReturnDate :  '미반품' }}</td>
              <td>{{ shoppingOrders.paymentStatus === 0 ?  '❌' : '⭕' }}</td>
              <td>
                ...
                <template v-else-if="shoppingOrders.salesState === 'deliveryComplete'">
                  <span style="color: green; font-weight: bold;">배송완료</span>
                </template>
                <template v-else>
                  <span></span>
                </template>
